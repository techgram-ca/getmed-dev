...
function groupStatus(orders: Order[]): 'assigned' | 'acknowledged' | 'picked_up' | 'mixed' {
  ...
  if (statuses.length === 1) return statuses[0] as 'assigned' | 'acknowledged' | 'picked_up' | 'mixed';
  return 'mixed';
}
...
export default function DriverDashboardPage() {
  ...
  const fetchOrders = useCallback(async () => {
    ...
    try {
      ...
      if (res.status === 401 || res.status === 403) { router.push('/driver/login'); return; }
  ...
  // Group-level action (acknowledge all / pickup all)
  const groupAction = async (pharmacyId: string, action: 'acknowledge' | 'pickup') => {
    setGroupActionId(`${pharmacyId}-${action}`);
    try {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.error);
      toast.success(action === 'acknowledge' ? 'Orders acknowledged!' : 'Orders marked as picked up!');
      fetchOrders();
    } catch (err: unknown) {
  ...
  // Per-order simple action (out_for_delivery, cancel)
  const orderAction = async (orderId: string, action: string) => {
    setOrderActionId(orderId + action);
    try {
      const fd = new FormData();
      fd.append('action', action);
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.error);
      toast.success(action === 'cancel' ? 'Order cancelled' : 'Status updated!');
      fetchOrders();
    } catch (err: unknown) {
      toast.error(err instanceof Error ? err.message : 'Action failed');
    } finally {
  ...
  const submitDelivery = async () => {
    ...
    if (modalMode === 'fail' && !failureReason) { toast.error('Please select a failure reason'); return; }
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.error);
      toast.success(modalMode === 'deliver' ? 'Delivered!' : 'Failure recorded');
      closeModal();
      fetchOrders();
    } catch (err: unknown) {
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <header className="bg-white border-b border-gray-200 px-4 py-4 flex items-center justify-between sticky top-0 z-10">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button variant="ghost" size="sm" onClick={handleLogout}><LogOut size={14} /> Sign Out</Button>
        </div>
      </header>
      ...
      <div className="max-w-2xl mx-auto px-4 py-6">
        {groups.length === 0 ? (
          ...
          <div className="space-y-6">
            {groups.map((group) => {
              const gs = groupStatus(group.orders);
              const allAssigned = group.orders.every((o) => o.status === 'assigned');
              const allAcknowledged = group.orders.every((o) => o.status === 'acknowledged');

              return (
                <div key={group.pharmacyId} className="bg-white rounded-xl border border-gray-200 overflow-hidden">
                  {/* Pharmacy pickup block */}
                  <div className="bg-teal-50 border-b border-teal-100 p-4">
                    <div className="flex items-start gap-3">
                      <div className="flex-1 min-w-0">
                        <p className="text-xs font-bold text-teal-700 uppercase tracking-wide mb-1">Pickup Location</p>
                        <p className="font-bold text-gray-900">{group.pharmacyName}</p>
                        <p className="text-sm text-gray-600 flex items-start gap-1 mt-0.5">
                          <MapPin size={13} className="shrink-0 mt-0.5" />{group.pharmacyAddress}
                        </p>
                        {group.pharmacyPhone && (
                          <a href={`tel:${group.pharmacyPhone}`} className="text-sm text-teal-600 flex items-center gap-1 mt-0.5 hover:underline">
                            <Phone size={13} />{group.pharmacyPhone}
                          </a>
                        )}
                        <p className="text-xs text-teal-600 font-medium mt-2">
                          {group.orders.length} delivery order{group.orders.length !== 1 ? 's' : ''}
                        </p>
                      </div>
                      <Button
                        size="sm"
                    ...
                    <div className="mt-3 flex gap-2 flex-wrap">
                      {allAssigned && (
                        <Button
                          size="sm"
                          loading={groupActionId === `${group.pharmacyId}-acknowledge`}
                          onClick={() => groupAction(group.pharmacyId, 'acknowledge')}
                          className="bg-indigo-600 hover:bg-indigo-700"
                        >
                          <CheckCircle size={14} /> Acknowledge All Orders
                        </Button>
                      )}
                      {allAcknowledged && (
                        <Button
                          size="sm"
                          loading={groupActionId === `${group.pharmacyId}-pickup`}
                          onClick={() => groupAction(group.pharmacyId, 'pickup')}
                          className="bg-cyan-600 hover:bg-cyan-700"
                        >
                          <ShoppingBag size={14} /> Picked Up from Pharmacy
                        </Button>
                      )}
                      {gs === 'mixed' && (
                        <Badge className="bg-orange-100 text-orange-700 text-xs px-2 py-1">
                          Orders in multiple stages
                        </Badge>
                      )}
                    </div>
                  </div>
                      ...
                      <div key={order.id} className="p-4">
                        <div className="flex items-start justify-between gap-2 mb-2">
                          <div className="flex-1 min-w-0">
                            ...
                            <a href={`tel:${order.patient_phone}`} className="text-sm text-gray-600 flex items-center gap-1 hover:text-teal-600">
                              <Phone size={13} />{order.patient_phone}
                            </a>
                          </div>
                          <p className="text-xs text-gray-400 shrink-0">{format(new Date(order.created_at), 'MMM d')}</p>
                        </div>

                        <div className="flex items-start gap-2 mb-3">
                          <MapPin size={14} className="text-gray-400 mt-0.5 shrink-0" />
                          <p className="text-sm text-gray-700 flex-1">{order.patient_address}</p>
                          <Button size="sm" variant="outline" onClick={() => openMaps(order.patient_address)} className="shrink-0 text-xs">
                        ...
                        <div className="flex gap-2 flex-wrap">
                          ...
                          {/* Out for Delivery — available once picked_up */}
                          {order.status === 'picked_up' && (
                            <Button
                              size="sm"
                              loading={orderActionId === order.id + 'out_for_delivery'}
                              onClick={() => orderAction(order.id, 'out_for_delivery')}
                          ...
                          {/* Deliver / Fail — available once out for delivery */}
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-end sm:items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-5">
              ...
              {modalMode === 'deliver' && (
                <div>
                  <label className="text-sm font-medium text-gray-700 block mb-1">Patient Signature (optional)</label>
                  <label className={cn(
                    'flex items-center gap-3 border-2 border-dashed rounded-lg px-4 py-3 cursor-pointer hover:border-teal-400 transition-colors',
            ...
            <div className="p-6 border-t border-gray-100 flex gap-3">
              ...
              <Button
                ...
                loading={submitting}
              >
                {modalMode === 'deliver' ? 'Confirm Delivered' : 'Report Failed'}
              </Button>
            </div>
          </div>
        </div>
